...
// Ikiye Bolme Yontemi'nin adimlarini iceren fonksiyon
void bisection(float coeff[N], double low_lim, double up_lim, double error, int degree)
{
	int iteration = 0;
	...
	do{
		...
		mid_lim = (low_lim + up_lim) / 2; // Orta deger, yontemdeki kurala gore hesaplanir.
		
		controlValue = (up_lim - low_lim) / pow(2, iteration); // Kontrol degeri hesaplanir.
		
		midRes = functionValue(coeff, mid_lim, degree); // Orta deger icin polinom degeri hesaplanir.
...
int main()
{
	...
	printf("\nEnter an error threshold: ");
	scanf("%lf", &error);
	
	bisection(coeff, low_lim, up_lim, error, degree);
